...
inline void HTools::computeDataMatrix(double* data_matrix, unsigned int num_points, double* points)
{
...
inline void HTools::crossprod(double *out, const double *a, const double *b, unsigned int st)
{
...
public:
	...
	std::vector<double> final_model_params_;// storage for the final result
...
inline bool HomogEstimator::initProblem(const ConfigParamsHomog& cfg, double* pointData)
{
...
inline void HomogEstimator::cleanupProblem()
{
	if (input_points_) { delete[] input_points_; input_points_ = NULL; }
	if (data_matrix_) { delete[] data_matrix_; data_matrix_ = NULL; }
	for (size_t i = 0; i < models_.size(); ++i)
...
// generateMinimalSampleModels: generates minimum sample model from the data points whose   indices are currently stored in min_sample_. In this case, only one model per minimum sample
inline unsigned int HomogEstimator::generateMinimalSampleModels()
{
	double A[8 * 9];
	double At[9 * 8];
...
inline bool HomogEstimator::generateRefinedModel(std::vector<unsigned int>& sample, unsigned int numPoints, bool weighted, double* weights)
{
...
inline bool HomogEstimator::validateSample()
{
...
inline bool HomogEstimator::validateModel(const unsigned int modelIndex)
{
	return true;
}
// evaluateModel: test model against all/subset of the data points
inline bool HomogEstimator::evaluateModel(unsigned int modelIndex, unsigned int* numInliers, unsigned int* numPointsTested)
{
...
inline void HomogEstimator::testSolutionDegeneracy(bool* degenerateModel, bool* upgradeModel)
{
...
inline unsigned int HomogEstimator::upgradeDegenerateModel()
{
	return 0;
}
// findWeights: given model and points, compute weights to be used in local optimization
inline void HomogEstimator::findWeights(unsigned int modelIndex, const std::vector<unsigned int>& inliers, unsigned int numInliers, double* weights)
{
...
inline void HomogEstimator::storeModel(const unsigned int modelIndex, unsigned int numInliers)
{
