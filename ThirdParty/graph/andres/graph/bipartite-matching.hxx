...
namespace andres {
	namespace graph {
		...
		inline int findMCBM(GRAPH const& graph, COST& costs, EDGELABELS& edge_labels);
		...
		inline int	findMCBM(GRAPH const& graph, COST& costs, EDGELABELS& edge_labels)
		{
			auto matcher = BipartiteMatching<GRAPH, COST, EDGELABELS>(graph, costs, edge_labels);
			matcher.run();
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>
		BipartiteMatching<GRAPH, COST, EDGELABELS>::BipartiteMatching(GRAPH const& graph, COST& costs, EDGELABELS& edge_labels) : graph_(graph), costs_(costs), edge_labels_(edge_labels)
		{
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	void	BipartiteMatching<GRAPH, COST, EDGELABELS>::run()
		{
			while (true) {
				switch (step_) {
				case Step::ONE:
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	std::vector<typename BipartiteMatching<GRAPH, COST, EDGELABELS>::IndexPair>	BipartiteMatching<GRAPH, COST, EDGELABELS>::matches() const
		{
			std::vector<IndexPair> matches;
			for (auto const& row : rows_)
			{
				for (auto it = graph_.adjacenciesFromVertexBegin(row); it != graph_.adjacenciesFromVertexEnd(row); ++it)
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_one()
		{
			value_type minval;
			...
			if (rows_.size() >= cols_.size())
			{
				...
				for (auto const& col : cols_)
				{
					...
					for (auto it = graph_.adjacenciesToVertexBegin(col); it != graph_.adjacenciesToVertexEnd(col); ++it)
					{
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_two()
		{
			for (auto const& row : rows_)
			{
				if (row_cover_[row])
				...
				for (auto it = graph_.adjacenciesFromVertexBegin(row); it != graph_.adjacenciesFromVertexEnd(row); ++it)
				{
					...
					// star zero entries.
					if (!col_cover_[it->vertex()] && !row_cover_[row] && costs_[it->edge()] == 0)
					{
		...
		/// columns are covered.
		///
		template <class GRAPH, class COST, class EDGELABELS>void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_three()
		{
			for (auto const& row : rows_)
				for (auto it = graph_.adjacenciesFromVertexBegin(row); it != graph_.adjacenciesFromVertexEnd(row); ++it)
					if (edge_labels_[it->edge()] == STARRED)
			...
			const std::size_t col_count = std::count(col_cover_.cbegin(), col_cover_.cend(), true);
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_four()
		{
			IndexPair idx;
		...
		///
		template <class GRAPH, class COST, class EDGELABELS>	void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_five()
		{
			while (true)
			{
				IndexPair  a = path_.back();
				auto const row = find_star_in_col(a.second);
				if (row == IDXMAX)
					break;

				a = path_.back();
				path_.emplace_back(row, a.second);
		...
		///
		template <class GRAPH, class COST, class EDGELABELS> void	BipartiteMatching<GRAPH, COST, EDGELABELS>::step_six()
		{
