...
namespace andres {
	namespace graph {
		...
		inline void sssp(const GRAPH& g, const SUBGRAPH_MASK& mask, const std::size_t vs, const EDGE_VALUE_ITERATOR edgeWeights, DISTANCE_ITERATOR distances, PARENT_ITERATOR parents, VISITOR& visitor)
		{
			typedef typename std::iterator_traits<DISTANCE_ITERATOR>::value_type Value;
			...
			const Value infinity = std::numeric_limits<Value>::has_infinity ? std::numeric_limits<Value>::infinity() : std::numeric_limits<Value>::max();
			...
			while (!queue.empty())
			{
				...
				for (typename GRAPH::AdjacencyIterator it = g.adjacenciesFromVertexBegin(v); it != g.adjacenciesFromVertexEnd(v); ++it)
				{
